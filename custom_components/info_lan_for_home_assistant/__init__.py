"""Version: 1.0.0. The Info-Lan integration."""
...
async def async_setup_entry(hass: HomeAssistant, entry: InfoLanConfigEntry) -> bool:
    ...
    client = InfoLanApiClient(hass=hass, login=entry.data[CONF_LOGIN], password=entry.data[CONF_PASSWORD])
    data_coordinator = InfoLanDataUpdateCoordinator(hass, client, entry)
    ...
    hass.data[DOMAIN][entry.entry_id] = {'client': client, 'coordinator': data_coordinator}
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload an Info-Lan config entry."""
